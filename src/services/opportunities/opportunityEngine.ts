import type { ProductProfit, CustomerProfit, BusinessMetrics } from '../metrics/metricsService';
import type { StockForecast } from '../inventory/inventoryForecast';
import { toNum } from '../../utils/number';

// Opportunity Center's detection engine. Every detector here is
// deterministic and reads only from data already computed elsewhere in the
// app (metrics engine, inventory forecasts, customer profiles) — no
// external AI call, no invented numbers. `estimatedImpact` is null whenever
// there isn't a real number to back it; a detector never guesses at a
// financial figure it can't actually compute from real records.
...
/** Unpaid sales, grouped by customer, each an opportunity to collect real money owed. */
...
/** Customers who used to order regularly but have gone quiet. */
export function detectDormantCustomers(customers: CustomerProfit[]): Opportunity[] {
  ...
  for (const c of customers) {
    ...
    if (c.transactionCount < 2) continue; // one-time buyers aren't "dormant regulars"
...
/** Products selling in real volume but at a thin margin relative to the rest of the catalog. */
export function detectLowMarginProducts(products: ProductProfit[]): Opportunity[] {
  ...
  for (const p of sellingProducts) {
    ...
    if (p.marginPct >= avgMargin) continue; // below the threshold but not actually below your own average — not worth flagging
...
/** Products at high risk of stocking out soon, using the same forecasts Smart Inventory shows. */
  ...
  for (const f of forecasts) {
    if (f.stockoutRisk === 'high' && f.unitsPerDay) {
      opportunities.push({
        ...
        estimatedImpact: null, // deliberately not estimating lost-sale revenue — can't know if a customer would have bought elsewhere or waited
...
/** Quotations sent out and never followed up — a classic dropped opportunity. */
...
/** A real, calculable jump in expenses period-over-period. */
