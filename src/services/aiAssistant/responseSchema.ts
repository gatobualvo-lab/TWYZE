// The structured-output contract for the AI Business Assistant. Claude is
// called with output_config.format set to RESPONSE_JSON_SCHEMA (no `tools`
// parameter at all) so the model returns exactly these fields — distinct
// facts/calculatedInsights/predictions/recommendations, rather than free
// prose the app would have to parse and hope was formatted consistently.
// parseAssistantResponse() defensively re-validates the parsed JSON before
// anything trusts it: a schema mismatch degrades to a 502, never gets
// stored or shown as if it were a normal answer.
...
/**
 * Validates a parsed Claude response against the expected shape. Returns
 * null (never throws) for anything malformed, so the Edge Function can
 * degrade to a 502 rather than store or return corrupted data.
 */
